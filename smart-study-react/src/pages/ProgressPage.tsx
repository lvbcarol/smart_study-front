          ...
          <div className="embla__viewport" ref={emblaRef}>
            <div className="embla__container">
              ...
              <div className="embla__slide p-4 flex flex-col lg:flex-row items-center gap-8">
                <div className="lg:w-2/3 w-full h-[28rem]">
                  <Card className="w-full h-full bg-white text-black">
                    <CardHeader><CardTitle>Overall Average by Notebook</CardTitle><CardDescription>A general comparison of your performance.</CardDescription></CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={300}><BarChart data={averageByNotebookData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 12 }} /><YAxis domain={[0, 100]} /><Tooltip cursor={{ fill: '#f3f4f6' }} /><Bar dataKey="averageScore" name="Average" fill="#a78bfa" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
                    </CardContent>
                  </Card>
                </div>
                <div className="lg:w-1/3 w-full text-center lg:text-left">
                  <h3 className="text-2xl font-bold">Subject Overview</h3>
                  <p className="mt-2 text-gray-300">This chart shows your overall average score for each notebook. Use it to quickly identify which subjects need more attention.</p>
                </div>
              </div>
              ...
              <div className="embla__slide p-4 flex flex-col lg:flex-row items-center gap-8">
                <div className="lg:w-2/3 w-full h-[28rem]">
                  <Card className="w-full h-full bg-white text-black">
                    <CardHeader>
                      ...
                      <div className="mt-4"><Select onValueChange={setNotebookForLessonAverage}><SelectTrigger><SelectValue placeholder="Select a Notebook" /></SelectTrigger><SelectContent>{allNotebooks.map(notebook => <SelectItem key={notebook._id} value={notebook._id}>{notebook.title}</SelectItem>)}</SelectContent></Select></div>
                    </CardHeader>
                    <CardContent>
                ...
                <div className="lg:w-1/3 w-full text-center lg:text-left">
                  <h3 className="text-2xl font-bold">Lesson Analysis</h3>
                  <p className="mt-2 text-gray-300">Filter by notebook to compare your average performance in each lesson, helping you pinpoint specific topics for review.</p>
                </div>
              </div>
              ...
              <div className="embla__slide p-4 flex flex-col lg:flex-row items-center gap-8">
                  ...
                  <Card className="w-full h-full bg-white text-black">
                    <CardHeader>
                      ...
                      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Select onValueChange={(value) => { setNotebookForAttempts(value); setLessonForAttempts(''); }}><SelectTrigger><SelectValue placeholder="Select a Notebook" /></SelectTrigger><SelectContent>{allNotebooks.map(notebook => <SelectItem key={notebook._id} value={notebook._id}>{notebook.title}</SelectItem>)}</SelectContent></Select>
                        <Select onValueChange={setLessonForAttempts} disabled={!notebookForAttempts} value={lessonForAttempts}><SelectTrigger><SelectValue placeholder="Select a Lesson" /></SelectTrigger><SelectContent>{selectedNotebookForAttempts?.lessons.map(lesson => <SelectItem key={lesson._id} value={lesson._id}>{lesson.title}</SelectItem>)}</SelectContent></Select>
                      </div>
                    </CardHeader>
                    <CardContent>
                        {attemptChartData.length > 0 ? (
                            <ResponsiveContainer width="100%" height={220}><BarChart data={attemptChartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis domain={[0, 100]} /><Tooltip cursor={{ fill: '#f3f4f6' }} />
                            {/* ✅ CORREÇÃO DA COR AQUI */}
                            <Bar dataKey="score" fill="#a78bfa" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
                ...
                <div className="md:w-1/3 w-full text-center md:text-left">
                  <h3 className="text-2xl font-bold">Deep Dive Analysis</h3>
                  <p className="mt-2 text-gray-300">Filter by notebook and lesson to see the score progression for every single attempt. Perfect for tracking improvement on a specific topic.</p>
                </div>
              </div>
